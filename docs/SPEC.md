...
cursor-style-blink = false         # 点滅が iTimeCursorChange をリセットするため（§9.3-6）
```
...
- [ ] 打鍵 → SMILE、放置 → IDLE → SLEEP が意図した秒数で遷移する（`cursor-style-blink = false` 必須。§9.3-6）
- [ ] 視線がカーソルを追う（上下が逆でない）
...
する。`err` は単一表情のまま（要件外）、SMILE（打鍵直後の顔）もバリアント対象外
（入力への即時反応は毎回同じ顔で返すほうが「反応した」ことが伝わるため）。

- 選択は `since` ではなく **`iTime` 基準**。`since` 基準だと打鍵のたびに表情が切り替わる。
...
自律視線は `smoothstep(1.5, 5.0, since)` で重み付けされており、**打鍵すると `since` が
リセットされて即カーソル注視に戻る**。ステートレスのまま「気づいてこちらを向く」反応に
なっている。

なお `gazeWander` と SMILE→IDLE→SLEEP はどちらも `since` に依存するため、カーソル点滅に
よる `iTimeCursorChange` のリセットの影響を受ける（§9.3-6。`cursor-style-blink = false` 推奨）。
