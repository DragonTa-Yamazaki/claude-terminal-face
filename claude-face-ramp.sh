...
if [ "${1:-}" = "--animate" ]; then
  ...
  #   body … 上式で w を 0.01→0.99 まで動かす区間（≈0.71s）。ここが実効モーフで、
  #           smoothstep の効く中央部 w=10%→90% は実測 ≈0.45s（VAR_MORPH ≈0.63s より
  #           明確に速い。2026-07-20、フェーズ間だけ短くする意図で 26→24→21 と調整）。
  #           1 ステップ ≈34ms が時間分解能なので、これ以上細かい狙い撃ちはできない。
